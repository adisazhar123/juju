...
const (
	appStorageConfigSummary = "Displays or sets storage constraints values on an application."
...
To view the value of a single storage key:

    juju application-storage <application> <storage-key>
...
Print the storage constraints for all keys of the postgresql application:

    juju application-storage postgresql

Print the storage constraints for the pgdata storage key of the postgresql application:

    juju application-storage postgresql pgdata

Set the size to 100GiB, pool name to "rootfs", and count to 1 for the mysql application:
...
If no pool is provided, Juju selects the default storage pool from the environment.
...
var applicationStorageConfigBase = config.ConfigCommandBase{
	Resettable: false,
}
...
func (c *storageConfigCommand) Info() *cmd.Info {
	info := &cmd.Info{
		...
		Args:     "<application-name> [<storage-key>[=<size>,<pool>,<count>]] ...",
...
func (c *storageConfigCommand) setConfig(client StorageConstraintsAPI, attrs config.Attrs) error {
	...
	return block.ProcessBlockedError(client.UpdateApplicationStorage(updateParams), block.BlockChange)
}
